fitPagel3 <- function (tree, x, y, method = "fitMk", model = "ARD", dep.var = "xy", 
          ...) 
{
  if (!inherits(tree, "phylo")) 
    stop("tree should be object of class \"phylo\".")
  if (dep.var %in% c("x", "y", "xy","xc", "yc", "xyc") == FALSE) {
  ...
  iQ <- matrix(c(0, 1, 2, 0, 3, 0, 0, 2, 0, 0, 0, 1, 0, 0, 
                 3, 0), 4, 4, byrow = TRUE)
  ...
  if (dep.var == "xyc") 
    dQ <- matrix(c(0, 1, 2, 0, 3, 0, 0, 4, 0, 0, 0, 6, 0, 
                   0, 8, 0), 4, 4, byrow = TRUE)
  else if (dep.var == "xc") 
    dQ <- matrix(c(0, 1, 2, 0, 3, 0, 0, 4, 0, 0, 0, 1, 0, 
                   0, 3, 0), 4, 4, byrow = TRUE)
  else if (dep.var == "yc") 
    dQ <- matrix(c(0, 1, 2, 0, 3, 0, 0, 2, 0, 0, 0, 5, 0, 
                   0, 6, 0), 4, 4, byrow = TRUE)
